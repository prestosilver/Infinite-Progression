...
#pragma warning restore 0649
    ...
    {
        return TakeObjectSnapshot(gameObject, Color.black, defaultPositionOffset, Quaternion.Euler(defaultRotation), defaultScale, width, height);
    }
    ...
    public Texture2D TakeObjectSnapshot(GameObject gameObject, Color backgroundColor, Vector3 positionOffset, Quaternion rotation, Vector3 scale, int width = 128, int height = 128)
    {
        ...
        {
            prevcam = gameObject.GetComponent<Canvas>().worldCamera;
        ...
        if (gameObject.GetComponent<Canvas>() != null)
            gameObject.GetComponent<Canvas>().worldCamera = prevcam;
